...
class OutletService extends AbstractService {
  ...
  getBrightness() {
    let brightness = this.getPropertyValue(this._brightnessProp());
    brightness = this.computeBrightnessMiredForGet(brightness);
    return brightness;
  }

  async setBrightness(value) {
    value = this.computeBrightnessMiredForSet(value);
  ...
  // 把设备真实数值缩放到 0-100 范围
  computeBrightnessMiredForGet(propValue) {
    let brightnessRange = this.getPropertyValueRange(this._brightnessProp());
    if (brightnessRange && brightnessRange.length > 2) {
      let maxVal = brightnessRange[1];
      if (maxVal > 0) {
        return Math.floor(propValue / maxVal * 100);
      }
    }
    return propValue;
  }

  computeBrightnessMiredForSet(wantValue) {
    if (wantValue > 0) {
      let brightnessRange = this.getPropertyValueRange(this._brightnessProp());
      if (brightnessRange && brightnessRange.length > 2) {
        let maxVal = brightnessRange[1];
        if (maxVal > 0) {
          return Math.floor(wantValue / 100 * maxVal);
        }
      }
    }
    return wantValue;
